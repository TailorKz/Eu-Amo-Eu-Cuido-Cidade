...
import { Image, StyleSheet, Text, TouchableOpacity, View, Modal, ActivityIndicator, ScrollView, Platform } from "react-native";
import { moderateScale, scale, verticalScale } from "../app/src/utils/responsive";
  ...
  const registrarParaPushNotifications = async () => {
    if (Device.isDevice) {
      ...
      try {
        ...
        if (user?.id) {
          await axios.put(`http://192.168.1.17:8080/api/cidadaos/${user.id}/push-token`, token, {
      ...
      console.log('Aviso: As notificações Push só funcionam num celular físico.');
    }
  };
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.banner}>
          ...
          <Image source={require("../assets/images/logoeuamoipora.png")} style={styles.logo} resizeMode="contain" />
        </View>
        ...
        <View style={styles.grid}>
          {isLoadingSetores ? (
            <ActivityIndicator size="large" color="#1F41BB" style={{ marginTop: 50, alignSelf: "center", width: "100%" }} />
          ) : (
            setores.map((item) => {
              const imageSource = imagensPadrao[item.nome] || { uri: item.icone };

              return (
                <TouchableOpacity
                  key={item.id}
                  style={styles.card}
                  onPress={() => router.push({ pathname: "/solicitacao", params: { categoria: item.nome } })}
                >
                  <Image source={imageSource} style={styles.cardImage} />
                  <View style={styles.textContainer}>
                    <Text style={styles.cardText} numberOfLines={2} adjustsFontSizeToFit>{item.nome}</Text>
                  </View>
                </TouchableOpacity>
              )
            })
      ...
      <Modal visible={modalAvisoVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TouchableOpacity style={styles.btnEntendi} onPress={() => setModalAvisoVisible(false)}>
...
const styles = StyleSheet.create({
  ...
  scrollContent: { paddingHorizontal: scale(16), paddingTop: verticalScale(40), paddingBottom: verticalScale(120) }, 
  
  headerGradient: { position: "absolute", top: 0, left: 0, right: 0, height: verticalScale(200), zIndex: 0 },
  banner: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", zIndex: 1 },
  name: { fontSize: moderateScale(24), fontWeight: "600", color: "#333" },
  logo: { width: scale(120), height: verticalScale(70) },
  title: { fontSize: moderateScale(24), fontWeight: "700", color: "#333", marginTop: verticalScale(20), marginBottom: verticalScale(20), textAlign: "center", zIndex: 1 },
  grid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", zIndex: 1 },
  card: { width: "48%", height: verticalScale(170), backgroundColor: "#FFF", borderRadius: moderateScale(14), alignItems: "center", marginBottom: verticalScale(10), elevation: 3 },
  cardImage: { width: scale(110), height: scale(110), marginTop: verticalScale(10) },
  cardText: { fontSize: moderateScale(18), fontWeight: "600", color: "#333", textAlign: "center", paddingHorizontal: 5 },
  textContainer: { flex: 1, justifyContent: "center", alignItems: "center", width: "100%" },

  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "center", alignItems: "center", padding: 20 },
  modalContent: { backgroundColor: "#FFF", width: "100%", borderRadius: 20, padding: 24, elevation: 10, alignItems: "center" },
  iconAviso: { width: 60, height: 60, borderRadius: 30, backgroundColor: "#FFF3E0", justifyContent: "center", alignItems: "center", marginBottom: 15 },
  modalTitle: { fontSize: 22, fontWeight: "bold", color: "#333", textAlign: "center", marginBottom: 10 },
  modalSubtitle: { fontSize: 16, color: "#555", textAlign: "center", marginBottom: 25, lineHeight: 22 },
  btnEntendi: { width: "100%", padding: 15, borderRadius: 12, backgroundColor: "#1F41BB", alignItems: "center" },
