...
export default function Perfil() {
  ...
  const API_URL = "http://192.168.1.17:8080/api/cidadaos";
  ...
  const salvarNovaSenha = async () => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/${user?.id}/alterar-senha?codigo=${codigoValidado}&novaSenha=${novaSenha}`);
  ...
  const solicitarCodigoNovoNumero = async () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/${user?.id}/solicitar-codigo?tipo=NUMERO&novoNumero=${numeroLimpo}`);
  ...
  const handleCodeConfirm = async (code: string) => {
    ...
    if (actionType === "password") {
      ...
      try {
        ...
        const response = await axios.put(`${API_URL}/${user?.id}/alterar-numero?codigo=${code}&novoNumero=${numeroLimpo}`);
      ...
      try {
        ...
        Alert.alert("Conta Excluída", "Sua conta e todos os seus dados foram apagados com sucesso.");
        logout();
        router.replace("/");
      } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.headerProfile}>
          ...
          <View style={styles.cityBadge}>
            ...
            <Text style={styles.cityText}>{user?.cidade || "Cidade não definida"}</Text>
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          <TouchableOpacity style={styles.row} activeOpacity={0.6} onPress={iniciarTrocaNumero} disabled={isLoading}>
            <View style={styles.rowLeft}>
              <View style={styles.iconBox}>
        ...
        <View style={styles.card}>
          <TouchableOpacity style={styles.row} activeOpacity={0.6} onPress={iniciarTrocaSenha} disabled={isLoading}>
            <View style={styles.rowLeft}>
              <View style={styles.iconBox}>
        ...
        <View style={styles.footerArea}>
          ...
          <TouchableOpacity style={styles.deleteAccountBtn} onPress={iniciarExclusaoConta} disabled={isLoading}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity style={styles.btnCancel} onPress={() => setIsSenhaAtualModalVisible(false)}>
                <Text style={styles.btnCancelText}>Cancelar</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.btnConfirm, actionType === "delete" && { backgroundColor: "#FF3B30" }]} 
                onPress={confirmarSenhaAtual}
                disabled={isLoading}
              >
                {isLoading ? <ActivityIndicator color="#FFF" /> : <Text style={styles.btnConfirmText}>Avançar</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      <Modal visible={isNovoNumeroModalVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>Enviaremos um SMS no seu Whatsapp para este novo número:</Text>
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity style={styles.btnCancel} onPress={() => setIsNovoNumeroModalVisible(false)}>
                <Text style={styles.btnCancelText}>Cancelar</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.btnConfirm} onPress={solicitarCodigoNovoNumero} disabled={isLoading}>
                {isLoading ? <ActivityIndicator color="#FFF" /> : <Text style={styles.btnConfirmText}>Avançar</Text>}
              </TouchableOpacity>
            </View>
          </View>
      ...
      <Modal visible={isNovaSenhaModalVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity style={styles.btnCancel} onPress={() => setIsNovaSenhaModalVisible(false)}>
                <Text style={styles.btnCancelText}>Cancelar</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.btnConfirm} onPress={salvarNovaSenha} disabled={isLoading}>
                {isLoading ? <ActivityIndicator color="#FFF" /> : <Text style={styles.btnConfirmText}>Salvar Senha</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  container: { paddingHorizontal: scale(16), paddingTop: verticalScale(10), paddingBottom: verticalScale(120) },
  pageTitle: { fontSize: moderateScale(24), fontWeight: "700", color: "#333", textAlign: "center", marginBottom: verticalScale(25) },
  headerProfile: { alignItems: "center", marginBottom: verticalScale(30) },
  avatar: { width: moderateScale(80), height: moderateScale(80), borderRadius: moderateScale(40), backgroundColor: "#1F41BB", justifyContent: "center", alignItems: "center", marginBottom: verticalScale(10), elevation: 3 },
  avatarText: { color: "#FFF", fontSize: moderateScale(30), fontWeight: "bold" },
  userName: { fontSize: moderateScale(22), fontWeight: "700", color: "#22", textTransform: 'capitalize' },
  cityBadge: { flexDirection: "row", alignItems: "center", backgroundColor: "#E4EBF7", paddingHorizontal: scale(12), paddingVertical: verticalScale(4), borderRadius: moderateScale(15), marginTop: verticalScale(6) },
  cityText: { marginLeft: scale(4), color: "#1F41BB", fontSize: moderateScale(13), fontWeight: "600" },
  sectionTitle: { fontSize: moderateScale(14), color: "#888", fontWeight: "600", marginLeft: scale(5), marginBottom: verticalScale(8), textTransform: "uppercase" },
  card: { backgroundColor: "#FFF", borderRadius: moderateScale(16), paddingVertical: verticalScale(5), marginBottom: verticalScale(25), elevation: 1 },
  row: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingVertical: verticalScale(12), paddingHorizontal: scale(16) },
  rowLeft: { flexDirection: "row", alignItems: "center" },
  iconBox: { width: moderateScale(36), height: moderateScale(36), borderRadius: moderateScale(10), backgroundColor: "#F0F4F8", justifyContent: "center", alignItems: "center", marginRight: scale(12) },
  label: { fontSize: moderateScale(15), color: "#333", fontWeight: "500" },
  value: { fontSize: moderateScale(15), color: "#666", fontWeight: "400" },
  valuePhone: { fontSize: moderateScale(14), color: "#1F41BB", fontWeight: "600", marginTop: verticalScale(2) },
  divider: { height: 1, backgroundColor: "#F0F0F0", marginLeft: scale(64) },
  footerArea: { marginTop: verticalScale(10), alignItems: "center" },
  footerLinks: { flexDirection: "row", alignItems: "center", marginBottom: verticalScale(25) },
  footerLinkText: { color: "#1F41BB", fontSize: moderateScale(14), fontWeight: "500" },
  footerDot: { color: "#CCC", marginHorizontal: scale(10), fontSize: moderateScale(16) },
  deleteAccountBtn: { paddingVertical: verticalScale(10), paddingHorizontal: scale(20), marginBottom: verticalScale(30) },
  deleteAccountText: { color: "#FF3B30", fontSize: moderateScale(15), fontWeight: "500", textDecorationLine: "underline" },
  logoutButton: { flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: "#3156df", width: "100%", paddingVertical: verticalScale(14), borderRadius: moderateScale(12) },
  logoutText: { marginLeft: scale(8), color: "#ffffff", fontSize: moderateScale(16), fontWeight: "600" },
  ...
  input: { backgroundColor: "#F3F4F6", borderRadius: 12, padding: 15, fontSize: 16, marginBottom: 15, borderWidth: 1, borderColor: "#E5E7EB" },
